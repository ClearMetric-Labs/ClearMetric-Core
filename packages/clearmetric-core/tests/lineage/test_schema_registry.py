...
def _sample_project() -> ProjectInput:
    fq = "analytics.input_layer.input_layer__condition"
...
def test_parse_canonical_relation_id() -> None:
    parts = parse_canonical_relation_id("analytics.core._stg_clinical_condition")
...
def test_normalize_short_name_to_canonical() -> None:
    ...
    assert resolved == "analytics.input_layer.input_layer__condition"
...
def test_schema_key_aliases_resolve_through_registry() -> None:
    canonical = "analytics.terminology.icd_10_cm"
    project = ProjectInput(
        input_kind="dbt_manifest",
        label="schema_aliases",
        datasets={
            ...
            "analytics.reference_data.calendar": ProjectDataset(
                name="analytics.reference_data.calendar",
                kind="root",
                sql=None,
                dependency_names=(),
    ...
    assert resolved_calendar.relation_id == "analytics.reference_data.calendar"
